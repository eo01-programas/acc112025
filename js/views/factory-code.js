/* ============================================================
   ...
   Migrado desde RESUMEN DE AUDITORÍA POR FACTORY CODE.html. Logica y marcado originales: el <script> se
    ...
    function mount(root) {
        ...
        function parseDateFromString(s) {
            ...
            if (s.indexOf('/') !== -1) {
                ...
                if (parts.length === 3) {
                    ...
                    if (month > 12 && day <= 12) {
                        ...
                        // day > 12 y month <= 12 => ya es dd/mm/yyyy, mantener como está
            ...
            if (s.indexOf('-') !== -1) {
                const parts = s.split('-').map(p => p.trim());
                if (parts.length === 3) {
                    // Si primer parte tiene 4 dígitos => yyyy-mm-dd
                    if (parts[0].length === 4) {
        ...
        // Cargar datos de ejemplo localmente para depuración
        ...
        // Helper: obtener lista de customers seleccionados (compatibilidad Select2 y select múltiple nativo)
        ...
        // Helper para el modal de gráfica (chartCustomerFilter)
        ...
        // Inicializar Select2 (se mantendrá deshabilitado hasta poblar opciones)
        if (window.jQuery) {
            __ready(function() {
            ...
            // Si no hay jQuery, asegurarse de que el select sea múltiple nativo
        ...
        if (yearSelect) yearSelect.addEventListener('change', function() { 
            populateWeekSelect();
            // Si populate estableció una semana/mes, actualizar tablas automáticamente
            try { if (weekSelect && weekSelect.value) handleWeekChange(); } catch(e) {}
        });
        if (periodSelect) periodSelect.addEventListener('change', function() { 
            populateWeekSelect();
            // Ajustar y actualizar tablas según la nueva selección de periodo
            try { if (weekSelect && weekSelect.value) handleWeekChange(); } catch(e) {}
        });
        ...
        function processSheetData(jsonData) {
            ...
            // Primero poblar años y periodo por defecto
            ...
            // Intentar seleccionar el año actual por defecto si existe en las opciones
            try {
                if (yearSelect) {
                    ...
                    if (hasCurrentYear) {
                        ...
                        // seleccionar el último año disponible
            ...
            // Poblar el select dependiente (week/month) después de fijar el año/periodo
            populateWeekSelect();
            console.log('[processSheetData] Week/month select populated');

            // Seleccionar por defecto la última semana con registros en el sheet
            try {
                if (periodSelect && periodSelect.value === 'Sem') {
                    // Obtener la última semana disponible (la de mayor número con registros)
                    if (weekSelect.options.length > 1) {
                        // La última opción es la semana más reciente (ya están ordenadas)
                ...
                // Disparar el filtrado por la selección por defecto
        ...
        function populateWeekSelect() {
            // Poblar el select dependiente según la columna 'Audit Date', el periodo y año seleccionados
            ...
            if (period === 'Sem') {
                ...
                // Meses: orden por número de mes 0..11
            ...
            // Ajustar etiqueta del switch dentro del fieldset Week según periodo
            try {
            ...
            // Si el periodo es Mes, intentar seleccionar automáticamente el mes que el usuario está viendo ahora.
            if (period === 'Mes') {
                // Si había una semana seleccionada previamente, usarla para inferir el mes
                ...
                // Si la opción existe en weekSelect, seleccionarla
                if (targetMonth) {
        ...
        function handleLastWeekToggle() {
            if (lastWeekToggle.checked) {
                // Guardar la selección actual para poder restaurarla al desactivar
                prevWeekSelection = weekSelect && weekSelect.value ? String(weekSelect.value) : null;

                // Determinar la semana base (la que el usuario está viendo ahora)
                const baseWeek = weekSelect && weekSelect.value ? String(weekSelect.value) : String(getCurrentWeekNumber());

                // Buscar el índice de la semana base en allWeeks
                ...
                if (idx > 0) {
                    ...
                    // Si baseWeek no está en allWeeks, intentar usar baseWeek-1 numérico
                ...
                // Restaurar la selección previa si existe, si no, intentar seleccionar la semana actual
        ...
        function handleWeekChange() {
            ...
            const weekData = gridData.filter(row => {
                ...
                // Fallback: if Audit Date no está parseable, intentar usar antigua columna Week para periodo Sem
            ...
            // Poblar el selector de Customer con valores únicos de ambas tablas
            ...
            // Solo aplicar filtros de Factory Code si hay algún switch activo o filtro de customer seleccionado
        ...
        function populateCustomerFilter(internaData, clienteData) {
            // Obtener todos los customers únicos de ambas tablas
            ...
            // Habilitar el selector (y avisar a Select2 si está activado)
            try {
                if (window.jQuery && $('#customerFilter').data('select2')) {
        ...
        // Poblar el filtro de Customers dentro del modal de gráfica
        function populateChartCustomerFilter() {
            const allCustomers = new Set();
            if (gridData && gridData.length > 0) {
                gridData.forEach(row => {
                    // Preferir filas tipo INTERNA (misma lógica que la gráfica)
        ...
        function applyFactoryFilters() {
            ...
            // Si no hay ningún filtro activo, mostrar todos los datos originales
        ...
        function renderTable(tbody, data, tableType) {
            ...
            data.forEach(row => {
                const factory = row['Factory Code'] || 'Sin código';
            ...
            // Columnas 21-24: vacías o con valores especiales para INTERNA
            if (tableType === 'INTERNA') {
                // Columna "Cuenta de Nº Report" - PASS
                ...
                // Columnas A2, A3, A4 vacías con fondo blanco
                for (let i = 0; i < 3; i++) {
                ...
                // Para CLIENTE, todas vacías con fondo blanco
                for (let i = 0; i < 4; i++) {
                    const cell = defectuosoRow.insertCell();
                    cell.textContent = '';
                    cell.style.backgroundColor = 'white';
            ...
            // Últimas columnas: vacías o con valores especiales para INTERNA
            if (tableType === 'INTERNA') {
                // Columna "Cuenta de Nº Report" - FAIL
            ...
            // === Resaltar en amarillo las 3 columnas de defectos con mayor % en la última fila ===
            ...
            // Calcular porcentaje numérico de cada defecto (cada defecto / Tot. Def.)
            const defectPercents = defectKeys.map((key, idx) => ({
                idx: idx,           // posición dentro de defectKeys (0-14)
                colIdx: idx + 5,    // índice de columna en la tabla (5-19)
        ...
        // Función para poblar los dropdowns de semanas
        function populateWeekFilters() {
            ...
            // Para periodo 'Mes' queremos mostrar los últimos 6 meses (mes actual y 5 anteriores).
            if (period === 'Mes') {
                ...
                // Unir y ordenar opciones para mostrar (asegurar que monthsNeeded estén presentes)
                ...
                // Seleccionar el rango monthsNeeded (primer y último)
                ...
                // Establecer valores por defecto (últimas 6 semanas si hay suficientes)
        ...
        // Función para poblar el dropdown de Factory Codes
        function populateFactoryFilter() {
            // Obtener Factory Codes únicos de los datos
            let factoryCodes = [];
            ...
            // Agregar cada Factory Code como opción
        ...
        // Función para actualizar la gráfica cuando cambia cualquier filtro
        function updateChartWithFilter() {
            ...
            // Validar que ambas semanas estén seleccionadas
            if (!weekFrom || !weekTo) {
        ...
        // Función para obtener datos de tendencias por rango de semanas
        ...
        // Función para obtener los datos de tendencias de las últimas 6 semanas
        function getWeeklyTrendsData(selectedFactory = 'all') {
            ...
            // Obtener las últimas 6 semanas disponibles en los datos
        ...
        // Función para obtener el porcentaje BAP (A1) de una semana específica
        function getBAPForWeek(weekNumber, selectedFactory = 'all') {
            ...
            // Denominador: conteo de filas donde Intento tiene dato (no vacío)
            ...
            // Filtrar filas por semana, tipo y factory (misma lógica que getTotalDefectsForWeek)
        ...
        // Función para obtener el porcentaje de defectuosos de la fila DEFECTUOSO para una semana específica
        ...
        // Función para obtener datos por mes (mesNumber 1..12)
        ...
        // Función para obtener BAP por mes (mesNumber 1..12)
        ...
        // Función para crear la gráfica de tendencias
        function createTrendsChart(data) {
            console.log('Creating chart with data:', data);
            console.log('BAP data array:', data.map(item => item.bap));
            
            // Usar los valores BAP calculados dinámicamente de los datos
            const bapValues = data.map(item => item.bap);
            console.log('Dynamic BAP values:', bapValues);
            
            // Calcular el máximo valor de % Defectuosos para ajustar el eje Y
                    ...
                    plugins: {
                        ...
                        datalabels: {
                            ...
                            color: function(context) {
                                return context.datasetIndex === 0 ? '#000000' : '#003d82'; // Negro para línea, azul oscuro para barras
                            },
                            font: {
                                family: 'Calibri',
                                size: 16,
                            ...
                            anchor: function(context) {
                                return context.datasetIndex === 0 ? 'end' : 'end'; // Línea: end, Barras: end
                            },
                            align: function(context) {
                                return context.datasetIndex === 0 ? 'bottom' : 'top'; // Línea: debajo, Barras: arriba
                            },
                            offset: 8
                        }
                    },
        ...
        function createStyledSheet(wb, tableId, sheetName) {
            ...
            // Para INTERNA: hasta columna Y (índice 24), para CLIENTE: hasta U (índice 20)
                    ...
                    if (R === 0) {
                        ...
                        if (C === 0 && typeof cellValue === 'string') {
                            if (cellValue.startsWith('Total ')) {
                                ...
                                // Fila DEFECTUOSO - aplicar formato porcentaje con 2 decimales desde columna E (índice 4) hasta T (índice 19)
                                for (let col = 0; col <= range.e.c; col++) {
                                    const addr = XLSX.utils.encode_cell({ r: R, c: col });
                                    ws[addr].s = JSON.parse(JSON.stringify(totalStyle));
                                    ...
                                    if (col >= 4 && col <= 19) {
                                        ...
                                        // Convertir tanto strings como números
                                        if (typeof cellVal === 'string') {
                                            // Si tiene %, quitar el símbolo
                                    ...
                                    // Para INTERNA: Columna V (índice 21) - formato % sin decimales
                            ...
                            // Detectar la última fila (siguiente después de DEFECTUOSO)
                            const prevRowFirstCell = XLSX.utils.encode_cell({ r: R - 1, c: 0 });
                            if (ws[prevRowFirstCell] && ws[prevRowFirstCell].v === 'DEFECTUOSO') {
                                // Última fila - formato porcentaje sin decimales desde columna D (índice 3) hasta T (índice 19)
                                for (let col = 0; col <= range.e.c; col++) {
                                    const addr = XLSX.utils.encode_cell({ r: R, c: col });
                        ...
                        // Hacer que los ceros tengan color blanco en filas de datos normales (no totales, no DEFECTUOSO) columnas E a Y (índices 4 a 24)
            ...
            for (let C = 0; C <= range.e.c; ++C) {
                if (C < 2) {
                    ...
                    colWidths.push({ wch: 6 }); // Resto de columnas C a U (índices 2 a 20)
                } else if (C >= 21 && C <= 24 && sheetName === 'INTERNA') {
                    colWidths.push({ wch: 4.5 }); // Columnas V, W, X, Y (índices 21-24) solo para INTERNA
                } else {
            ...
            // Para CLIENTE: eliminar columnas V, W, X (índices 21, 22, 23)
            if (sheetName === 'CLIENTE') {
                ...
                // Ajustar el rango para terminar en columna U (índice 20)
                range.e.c = 20;
                ws['!ref'] = XLSX.utils.encode_range(range);
                // Ajustar anchos de columna
        ...
        function initializeApp() {
            ...
            btnDownload.disabled = true; // Asegurar que esté deshabilitado durante la carga
            btnChart.disabled = true; // Asegurar que esté deshabilitado durante la carga
            ...
            loadSheetJSONP(SHEET_ID, SHEET_NAME)
                ...
                .catch(error => {
                    ...
                    showError('No se pudieron cargar los datos desde Google Sheets. Verifique que la hoja sea pública y tenga el nombre correcto.');
