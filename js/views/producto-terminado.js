...
(function () {
    ...
    var FILTERS_STORE_KEY = 'producto-terminado.filters';
    ...
    function mount(root) {
        ...
        // Header compacto (igual que Factory Code): Excel y Gráficos pasan a botones
        // circulares a la izquierda de Inicio, y los filtros suben a la línea del título.
        function _iconBtnPT(sel, svg, bg, bgHover, title) {
        ...
        var _svgDownPT = "<svg width='20' height='20' viewBox='0 0 24 24' fill='#fff'><path d='M5 20h14v-2H5v2zM19 9h-4V3H9v6H5l7 7 7-7z'/></svg>";
        var _svgBarsPT = "<svg width='20' height='20' viewBox='0 0 24 24' fill='#fff'><rect x='3' y='11' width='4' height='9' rx='1'/><rect x='10' y='6' width='4' height='14' rx='1'/><rect x='17' y='3' width='4' height='17' rx='1'/></svg>";
        _iconBtnPT('#btnDownload', _svgDownPT, 'var(--btn-excel-bg, #f78e6d)', 'var(--btn-excel-bg-hover, #d1603e)', 'Descargar Excel');
        _iconBtnPT('#btnCharts', _svgBarsPT, 'var(--btn-chart-bg, #29b6f6)', 'var(--btn-chart-bg-hover, #0d47a1)', 'Gráficos');
        var _hdrPT = root.querySelector('.header-title');
        var _ctrlPT = root.querySelector('.controls');
        var _filtersPT = root.querySelector('.filters-row');
        var _btnRowPT = root.querySelector('#btnDownload') && root.querySelector('#btnDownload').parentElement;
        if (_hdrPT && _ctrlPT && _homeBtn && _filtersPT && _btnRowPT) {
            _hdrPT.insertBefore(_filtersPT, _homeBtn);
            _hdrPT.insertBefore(_btnRowPT, _homeBtn);
            // Agrupar Excel + Gráficos + Inicio para que se mantengan juntos.
            _btnRowPT.style.gap = '8px'; _btnRowPT.style.flexWrap = 'nowrap';
            _btnRowPT.appendChild(_homeBtn);
            // controls-left (carga de archivo) sigue oculto por CSS; se conserva en el DOM
            // porque loadingStatus/fileInput ya tienen listeners.
            var _leftPT = _ctrlPT.querySelector('.controls-left');
            if (_leftPT) _hdrPT.appendChild(_leftPT);
            _ctrlPT.remove();
            _hdrPT.style.justifyContent = 'flex-start';
            _hdrPT.style.flexWrap = 'wrap'; _hdrPT.style.gap = '6px 10px';
            _hdrPT.style.marginBottom = '16px';
            var _h1PT = _hdrPT.querySelector('h1');
            if (_h1PT) {
                _h1PT.style.flex = '0 0 auto';
                _h1PT.style.lineHeight = '1.15'; _h1PT.style.letterSpacing = '0.03em'; _h1PT.style.margin = '0';
            }
            // Filtros pegados a la derecha, junto a los botones (como Factory Code).
            _filtersPT.style.marginLeft = 'auto';
            _filtersPT.style.alignItems = 'center';
            _filtersPT.style.flexWrap = 'nowrap';
            Array.prototype.forEach.call(_filtersPT.querySelectorAll('.switch-label'), function (el) {
                el.style.minWidth = '0';
            });
        }
        // Botón ⟳: fuerza la recarga de datos saltándose el caché de 5 minutos.
        // initializeApp está declarada más abajo dentro de mount (hoisting).
        if (_btnRowPT && _homeBtn && !root.querySelector('#btnRefreshPT')) {
            var _rbPT = document.createElement('button');
            _rbPT.id = 'btnRefreshPT';
            _btnRowPT.insertBefore(_rbPT, _homeBtn);
            _iconBtnPT('#btnRefreshPT', "<svg width='20' height='20' viewBox='0 0 24 24' fill='#fff'><path d='M17.65 6.35A7.95 7.95 0 0012 4a8 8 0 108 8h-2a6 6 0 11-1.76-4.24L13 11h7V4l-2.35 2.35z'/></svg>", 'var(--sc8-primary)', 'var(--sc8-primary-dark)', 'Actualizar datos desde Google Sheets');
            _rbPT.style.cursor = 'pointer';
            _rbPT.addEventListener('click', function () { initializeApp(true); });
        }
        ...
        // Cerrar modal al hacer clic fuera de él. Handler guardado en App para
        // quitar el del montaje anterior (antes se acumulaba uno por visita).
        if (App._ptModalWinClick) window.removeEventListener('click', App._ptModalWinClick);
        App._ptModalWinClick = function (event) {
            if (event.target === chartsModal) {
                closeChartsModal();
            }
        };
        window.addEventListener('click', App._ptModalWinClick);
        ...
        // Factory switches: re-aplican los filtros al cambiar
        if (cofacoSwitch) cofacoSwitch.addEventListener('change', applyFilters);
        if (cititex1Switch) cititex1Switch.addEventListener('change', applyFilters);
        if (cititex2Switch) cititex2Switch.addEventListener('change', applyFilters);
                        ...
                        if (typeof dateStr === 'number') {
                            ...
                            // Manejar formato específico Date(yyyy,m,d)
                                ...
                                if (dateMatch) {
                                    ...
                                    // En Date(2025,10,8) el mes ya viene en base-0 de
                                    // JavaScript (10 = noviembre): NO se resta 1.
                                    const monthFromDate = parseInt(dateMatch[2]);
        ...
        function persistFilters() {
            const state = {
                from: dateFromFilter ? (dateFromFilter.value || '') : '',
                to: dateToFilter ? (dateToFilter.value || '') : '',
                hoy: !!(todaySwitch && todaySwitch.checked),
                ayer: !!(yesterdaySwitch && yesterdaySwitch.checked),
                cofaco: !!(cofacoSwitch && cofacoSwitch.checked),
                c1: !!(cititex1Switch && cititex1Switch.checked),
                c2: !!(cititex2Switch && cititex2Switch.checked)
            };
            try { sessionStorage.setItem(FILTERS_STORE_KEY, JSON.stringify(state)); } catch (e) {}
            if (location.hash.indexOf('#/producto-terminado') !== 0) return;
            const params = new URLSearchParams();
            if (state.from) params.set('from', state.from);
            if (state.to) params.set('to', state.to);
            if (state.hoy) params.set('hoy', '1');
            if (state.ayer) params.set('ayer', '1');
            if (state.cofaco) params.set('cofaco', '1');
            if (state.c1) params.set('c1', '1');
            if (state.c2) params.set('c2', '1');
            try { history.replaceState(null, '', '#/producto-terminado?' + params.toString()); } catch (e) {}
        }
        ...
        function readSavedFilters() {
            ...
            if (qIdx > -1 && location.hash.indexOf('#/producto-terminado') === 0) {
                const params = new URLSearchParams(location.hash.slice(qIdx + 1));
                if (params.get('from') || params.get('to') || params.get('hoy') || params.get('ayer') || params.get('cofaco') || params.get('c1') || params.get('c2')) {
                    return {
                        from: params.get('from') || '',
                        to: params.get('to') || '',
                        hoy: params.get('hoy') === '1',
                        ayer: params.get('ayer') === '1',
        ...
        function restoreSavedFilters() {
            ...
            try {
                dateFromFilter.value = saved.from || '';
                dateToFilter.value = saved.to || '';
                todaySwitch.checked = !!saved.hoy;
                yesterdaySwitch.checked = !!saved.ayer;
                cofacoSwitch.checked = !!saved.cofaco;
                cititex1Switch.checked = !!saved.c1;
                cititex2Switch.checked = !!saved.c2;
                applyFilters();
            } catch (e) {
                return false;
            }
        ...
        // `forceReload` (botón ⟳) salta el caché de 5 minutos.
        function initializeApp(forceReload) {
            const cacheValido = forceReload !== true && _dataCache.rows && (Date.now() - _dataCache.loadedAt) < DATA_TTL_MS;
            if (cacheValido) {
                processSheetData(_dataCache.rows);
                renderTable();
                // Restaurar la última selección; si no hay, los defaults de siempre
                if (!restoreSavedFilters()) populateDateFilters();
            ...
            loadSheetJSONP(SHEET_ID, SHEET_NAME)
                .then(data => {
                    _dataCache = { rows: data, loadedAt: Date.now() };
        ...
        function processChartData(data) {
            ...
            // Procesar datos por fecha dentro del rango de 14 días
            data.forEach(row => {
