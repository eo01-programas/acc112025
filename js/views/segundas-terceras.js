...
(function () {
    ...
    function populateFilters(rows) {
        ...
        function populatePeriodOptions() {
            ...
            if (group === 'SEMANA') {
                const set = new Set();
                (rows || []).forEach(r => { const d = tryParseDate(r[fechaKey]); if (!d) return; const y = d.getFullYear(); const w = getISOWeek(d); set.add(`${y}|${String(w).padStart(2,'0')}`); });
                const arr = Array.from(set).sort();
                const optAll = document.createElement('option'); optAll.value = ''; optAll.textContent = 'Todas'; periodSel.appendChild(optAll);
                arr.forEach(k => { const [y,w] = k.split('|'); const o = document.createElement('option'); o.value = k; o.textContent = `${y}-SEM${Number(w)}`; periodSel.appendChild(o); });
                if (arr.length) periodSel.value = arr[arr.length - 1];
            } else {
                const set = new Set();
                const monthsNames = ['Ene','Feb','Mar','Abr','May','Jun','Jul','Ago','Sep','Oct','Nov','Dic'];
                (rows || []).forEach(r => { const d = tryParseDate(r[fechaKey]); if (!d) return; const y = d.getFullYear(); const m = String(d.getMonth()+1).padStart(2,'0'); set.add(`${y}|${m}`); });
                const arr = Array.from(set).sort();
                const optAll = document.createElement('option'); optAll.value = ''; optAll.textContent = 'Todas'; periodSel.appendChild(optAll);
                arr.forEach(k => { const [y,m] = k.split('|'); const o = document.createElement('option'); o.value = k; o.textContent = `${monthsNames[Number(m)-1]} ${y}`; periodSel.appendChild(o); });
                if (arr.length) periodSel.value = arr[arr.length - 1];
            }
        }
