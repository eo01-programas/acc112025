...
(function () {
    ...
    // Mismo patrón que depurado y segundas-terceras: TTL 5 min + botón ⟳.
    var DATA_TTL_MS = 5 * 60 * 1000;
    var _dataCache = { bdcof: null, ingembcof: null, bdcofprod: null, loadedAt: 0 };
    var FILTERS_STORE_KEY = 'reporte-inspeccion.filters';
    ...
    function mount(root) {
        ...
        if (_actRI && !root.querySelector('#btnRefreshCircle')) {
            var _rbRI = document.createElement('button');
            _rbRI.id = 'btnRefreshCircle';
            _rbRI.className = 'btn-chart-circle';
            _rbRI.title = 'Actualizar datos desde Google Sheets';
            _rbRI.innerHTML = "<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='currentColor'><path d='M17.65 6.35A7.95 7.95 0 0012 4a8 8 0 108 8h-2a6 6 0 11-1.76-4.24L13 11h7V4l-2.35 2.35z'/></svg>";
            _actRI.insertBefore(_rbRI, root.querySelector('.back-btn'));
            _rbRI.addEventListener('click', function () { loadData(true); });
        }
    ...
    function populateFilters(rows) {
        ...
        function populatePeriodOptions() {
            ...
            if (group === 'SEMANA') {
                const weeks = getWeeksForYear(dataRows, year, fk);
                weeks.forEach(w => {
                    const opt = document.createElement('option');
                ...
                const months = getMonthsForYear(dataRows, year, fk);
        ...
        if (lineaSel) {
            ...
            if (!lineaSel.dataset.bound) {
                lineaSel.dataset.bound = '1';
    ...
    function filterIngresoRowsByPeriod(rows, group, period) {
        ...
        if (group === 'SEMANA') {
            ...
            return filterByPeriod(rows, 'MES', period);
        }
    }
    ...
    function applyFiltersAndBuild() {
        ...
        persistFilters(group, period, lineaVal);
    ...
    function persistFilters(group, period, lineaVal) {
        const yearSel = document.getElementById('filterYear');
        const state = {
        ...
        if (location.hash.indexOf('#/reporte-inspeccion') !== 0) return;
        const params = new URLSearchParams();
        if (state.year) params.set('year', state.year);
        params.set('g', state.groupBy);
        if (state.period) params.set('p', state.period);
        if (state.linea) params.set('l', state.linea);
        try { history.replaceState(null, '', '#/reporte-inspeccion?' + params.toString()); } catch (e) {}
    }
    ...
    function readSavedFilters() {
        ...
        if (qIdx > -1 && location.hash.indexOf('#/reporte-inspeccion') === 0) {
            const params = new URLSearchParams(location.hash.slice(qIdx + 1));
            if (params.get('year') || params.get('g') || params.get('p') || params.get('l')) {
                return {
    ...
    // Cambiar el año (dispatch) repuebla los periodos con los listeners existentes.
    function restoreSavedFilters() {
        ...
        // El periodo guardado, si sigue existiendo; si no, queda el último del año
        if (periodSel && hasOption(periodSel, saved.period)) periodSel.value = saved.period;
        if (lineaSel && (!saved.linea || hasOption(lineaSel, saved.linea))) {
            lineaSel.value = saved.linea || '';
        }
     ...
     * INICIO
     *************************************************************/
    // Filtro por defecto (lógica original): la semana más antigua entre la
    // última de `bdcof` y la última con dato de `bdcofprod`.
    ...
    function loadData(forceReload) {
        const cacheValido = !forceReload && _dataCache.bdcof && (Date.now() - _dataCache.loadedAt) < DATA_TTL_MS;
        if (cacheValido) {
            _originalBdcof = _dataCache.bdcof;
            _originalIngembcof = _dataCache.ingembcof;
            _originalBdcofprod = _dataCache.bdcofprod;
            setStatus("Datos cargados", "ok");
            populateFilters(_originalBdcof);
            if (!restoreSavedFilters()) applyDefaultPeriod();
            return;
        }
        ...
        .then(([bdcofRows, ingembcofRows, bdcofprodRows]) => {
                ...
                _dataCache = { bdcof: _originalBdcof, ingembcof: _originalIngembcof, bdcofprod: _originalBdcofprod, loadedAt: Date.now() };
                setStatus("Datos cargados", "ok");
                populateFilters(_originalBdcof);
                // Restaurar la última selección; si no hay, el default original
                if (!restoreSavedFilters()) applyDefaultPeriod();
        })
        .catch(err => {
