...
(function () {
    ...
    function mount(root) {
        ...
        function renderCharts(sums) {
            try {
                ...
                function groupHtml(title, data) {
                    const groupClass = title && title.toUpperCase().includes('ACABADOS') ? 'group-charts group-acabados' : 'group-charts';
                        ...
                        function showChartForEquipoLink(linkEl, eqNum) {
                            ...
                            if (!hasData) { ctx.clearRect(0, 0, canvas.width, canvas.height); ctx.font = '12px Arial'; ctx.fillStyle = '#333'; ctx.fillText('No hay datos históricos para este equipo', 8, 20); }
                                ...
                                window._eqTooltipChart = new Chart(ctx, {
                                    ...
                                    options: {
                                        responsive: false, maintainAspectRatio: false, scales: { x: { ticks: { autoSkip: false } }, yDef: { type: 'linear', position: 'left', ticks: { callback: v => v + '%' } }, yNivel: { type: 'linear', position: 'right', min: 0, max: 100, grid: { drawOnChartArea: false }, ticks: { callback: v => v + '%' } } }, plugins: { legend: { display: false } }
                                    }
                                });
                            }
        ...
        // Cargar hoja vía JSONP usando Google Visualization (evita problemas CORS)
        ...
        async function loadAndCompute() {
            ...
            try {
                ...
                // Keep badge as check symbol and ensure tooltip
                const lbl = document.getElementById('filterLabel');
                if (lbl) { lbl.innerHTML = '✓'; lbl.title = 'Check Datos'; lbl.setAttribute('aria-label', 'Check Datos'); }
        ...
        function renderPerfChartCanvas(ctx, labels, defArr, bapArr) {
            ...
            // plugin dibuja etiquetas encima de cada barra/punto
                ...
                afterDatasetsDraw(chart, args, options) {
                    ...
                    chart.data.datasets.forEach((dataset, datasetIndex) => {
                        ...
                        for (let i = 0; i < meta.data.length; i++) {
                            ...
                            const rawVal = dataset.data[i];
                            if (rawVal === null || rawVal === undefined) continue;
                            const valText = (typeof rawVal === 'number') ? rawVal.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 2 }) + '%' : String(rawVal);
                            const x = el.x;
                            let y = (el.y !== undefined) ? el.y : (el.base !== undefined ? el.base : 0);
                            // estilo y posición según tipo
                            if (dataset.type === 'bar' || datasetIndex === 0) {
                                y = el.y - 6; // encima de la barra
                                ctx.fillStyle = '#c9172e'; // rojo para barras
                            } else {
                                y = el.y - 10; // encima del punto
                                ctx.fillStyle = '#0b3d91'; // azul oscuro para puntos
                            }
            ...
            const cfg = {
                ...
                data: {
                    labels: labels,
                    datasets: [
                        { type: 'bar', label: '%BAP', data: bapArr, yAxisID: 'yBap', backgroundColor: 'rgba(59,130,246,0.4)' },
                        { type: 'line', label: '%Def.', data: defArr, yAxisID: 'yDef', borderColor: 'rgba(203, 23, 46, 0.9)', backgroundColor: 'rgba(203,23,46,0.08)', tension: 0.2, pointRadius: 4, fill: false }
                    ]
                },
                options: {
                    ...
                    scales: {
                        x: { stacked: false },
                        yDef: { type: 'linear', position: 'left', title: { display: true, text: '%Def.' }, ticks: { callback: v => v + '%' } },
                        yBap: { type: 'linear', position: 'right', title: { display: true, text: '%BAP' }, grid: { drawOnChartArea: false }, ticks: { callback: v => v + '%' } }
                    },
