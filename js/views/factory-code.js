...
(function () {
    ...
    // Mismo patrón que depurado/segundas-terceras/reporte-inspeccion.
    var DATA_TTL_MS = 5 * 60 * 1000;
    var _dataCache = { rows: null, loadedAt: 0 };
    var FILTERS_STORE_KEY = 'factory-code.filters';
    ...
    function mount(root) {
        ...
        if (_hdrFC && _ctrlFC && _homeBtn) {
            ...
            // Sin wrap: título + filtros + botones deben caber en UNA sola fila
            // (título compactado a 22px y gaps reducidos para entrar a ~1366px).
            _hdrFC.style.flexWrap = 'nowrap'; _hdrFC.style.gap = '6px 10px';
            var _h1FC = _hdrFC.querySelector('h1');
            if (_h1FC) {
                _h1FC.textContent = 'AUDITORIAS POR FACTORY';
                _h1FC.style.fontSize = '22px'; _h1FC.style.whiteSpace = 'nowrap';
                _h1FC.style.lineHeight = '1.1'; _h1FC.style.letterSpacing = '0.03em'; _h1FC.style.margin = '0';
                _h1FC.style.flexShrink = '0';
            }
            var _clFC = _hdrFC.querySelector('.controls-left');
            if (_clFC) {
                _clFC.style.flexWrap = 'nowrap';
                _clFC.style.gap = '8px';
                _clFC.style.minWidth = '0';
            }
        }
        ...
        if (_bcFC && _homeBtn) {
            _bcFC.style.flexWrap = 'nowrap';
            _bcFC.style.flexShrink = '0';
            _bcFC.style.gap = '6px';
            _bcFC.appendChild(_homeBtn);
        }
        ...
        if (_bcFC && !root.querySelector('#btnRefreshFC')) {
            var _rbFC = document.createElement('button');
            _rbFC.id = 'btnRefreshFC';
            _bcFC.insertBefore(_rbFC, _homeBtn);
            _iconBtnFC('#btnRefreshFC', "<svg width='20' height='20' viewBox='0 0 24 24' fill='#fff'><path d='M17.65 6.35A7.95 7.95 0 0012 4a8 8 0 108 8h-2a6 6 0 11-1.76-4.24L13 11h7V4l-2.35 2.35z'/></svg>", 'var(--sc8-primary)', 'var(--sc8-primary-dark)', 'Actualizar datos desde Google Sheets');
            _rbFC.style.cursor = 'pointer';
            _rbFC.addEventListener('click', function () { initializeApp(true); });
        }
        // Botones a 36px (los 4) para asegurar la fila única a ~1366px de ancho.
        ['#btnDownload', '#btnChart', '#btnRefreshFC', '.back-btn-h1'].forEach(function (s) {
        ...
        (function () {
            ...
            // Guardar el handler en App para quitar el del montaje anterior
            // (antes se acumulaba un listener sobre document por cada visita).
            if (App._fcCustDropClose) document.removeEventListener('click', App._fcCustDropClose);
            App._fcCustDropClose = function () { _drop.style.display = 'none'; };
            document.addEventListener('click', App._fcCustDropClose);
        ...
        (function () {
            ...
            // Handler en App para no acumular un listener por montaje (leak).
            if (App._fcChartDropClose) document.removeEventListener('click', App._fcChartDropClose);
            App._fcChartDropClose = function () { _drop.style.display = 'none'; };
            document.addEventListener('click', App._fcChartDropClose);
            _drop.addEventListener('click', function (e) { e.stopPropagation(); });
        ...
        // Cerrar modal al hacer click fuera de �l. Handler guardado en App para
        // quitar el del montaje anterior (antes se acumulaba uno por visita).
        if (App._fcModalWinClick) window.removeEventListener('click', App._fcModalWinClick);
        App._fcModalWinClick = function (event) {
            if (event.target === chartModal) {
                hideModal();
            }
        };
        window.addEventListener('click', App._fcModalWinClick);
        ...
        // El filtro de Customer (dropdown múltiple) no se persiste.
        function persistFilters() {
            const state = {
                year: yearSelect ? (yearSelect.value || '') : '',
                period: periodSelect ? (periodSelect.value || 'Sem') : 'Sem',
                week: weekSelect ? (weekSelect.value || '') : '',
                cofaco: !!(cofacoToggle && cofacoToggle.checked),
                cititex1: !!(cititex1Toggle && cititex1Toggle.checked),
                cititex2: !!(cititex2Toggle && cititex2Toggle.checked)
            };
            try { sessionStorage.setItem(FILTERS_STORE_KEY, JSON.stringify(state)); } catch (e) {}
            if (location.hash.indexOf('#/factory-code') !== 0) return;
            const params = new URLSearchParams();
            if (state.year) params.set('year', state.year);
            params.set('per', state.period);
            if (state.week) params.set('w', state.week);
            if (state.cofaco) params.set('cofaco', '1');
            if (state.cititex1) params.set('c1', '1');
            if (state.cititex2) params.set('c2', '1');
            try { history.replaceState(null, '', '#/factory-code?' + params.toString()); } catch (e) {}
        }
        ...
        function readSavedFilters() {
            ...
            if (qIdx > -1 && location.hash.indexOf('#/factory-code') === 0) {
                ...
                if (params.get('year') || params.get('per') || params.get('w')) {
                    return {
                        ...
                        cititex1: params.get('c1') === '1',
                        cititex2: params.get('c2') === '1'
                    };
                }
            }
            try {
        ...
        function restoreSavedFilters() {
            ...
            try {
                if (periodSelect && (saved.period === 'Sem' || saved.period === 'Mes')) periodSelect.value = saved.period;
                if (yearSelect && saved.year && hasOption(yearSelect, saved.year)) yearSelect.value = saved.year;
                populateWeekSelect();
                if (weekSelect && saved.week && hasOption(weekSelect, saved.week)) weekSelect.value = saved.week;
                if (cofacoToggle) cofacoToggle.checked = !!saved.cofaco;
                if (cititex1Toggle) cititex1Toggle.checked = !!saved.cititex1;
                if (cititex2Toggle) cititex2Toggle.checked = !!saved.cititex2;
                // handleWeekChange renderiza y aplica los filtros de planta activos
                if (weekSelect && weekSelect.value) handleWeekChange();
            } catch (e) {
                return false;
            }
            return true;
        ...
        function initializeApp(forceReload) {
            ...
            loadSheetJSONP(SHEET_ID, SHEET_NAME)
                .then(data => {
                    ...
                    // Restaurar la última selección sobre los defaults recién aplicados
                    restoreSavedFilters();
                })
                .catch(error => {
