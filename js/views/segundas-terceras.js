...
(function () {
    ...
    function populateFilters(rows) {
        ...
        function populatePeriodOptions() {
            ...
            if (group === 'SEMANA') {
                const set = new Set();
                (rows || []).forEach(r => { const d = tryParseDate(r[fechaKey]); if (!d) return; const y = d.getFullYear(); if (selYear && String(y) !== String(selYear)) return; const w = getISOWeek(d); set.add(`${y}|${String(w).padStart(2,'0')}`); });
                ...
                (rows || []).forEach(r => { const d = tryParseDate(r[fechaKey]); if (!d) return; const y = d.getFullYear(); if (selYear && String(y) !== String(selYear)) return; const m = String(d.getMonth()+1).padStart(2,'0'); set.add(`${y}|${m}`); });
                const arr = Array.from(set).sort();
        ...
        if (yearSel) yearSel.addEventListener('change', () => { populatePeriodOptions(); applyFiltersAndBuild(); });
