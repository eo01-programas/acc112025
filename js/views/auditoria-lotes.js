...
(function () {
    ...
    var FILTERS_STORE_KEY = 'auditoria-lotes.filters';
    ...
    function mount(root) {
        ...
        // loadAndCompute/showLoading están declaradas más abajo dentro de mount (hoisting).
        var _perfBtnAL = root.querySelector('#perfBtn');
        if (_perfBtnAL && !root.querySelector('#refreshBtnAL')) {
            var _rbAL = document.createElement('button');
            _rbAL.id = 'refreshBtnAL';
            _rbAL.type = 'button';
            _rbAL.className = 'perf-btn';
            _rbAL.title = 'Actualizar datos desde Google Sheets';
            _rbAL.setAttribute('aria-label', 'Actualizar datos');
            _rbAL.innerHTML = "<svg viewBox='0 0 24 24' fill='white' xmlns='http://www.w3.org/2000/svg' aria-hidden='true' focusable='false'><path d='M17.65 6.35A7.95 7.95 0 0012 4a8 8 0 108 8h-2a6 6 0 11-1.76-4.24L13 11h7V4l-2.35 2.35z'/></svg>";
            _perfBtnAL.parentNode.insertBefore(_rbAL, _perfBtnAL.nextSibling);
            _rbAL.addEventListener('click', function () { showLoading('Actualizando datos...'); loadAndCompute(true); });
        }
        ...
        function renderCharts(sums) {
            try {
                ...
                function groupHtml(title, data) {
                    ...
                    const groupClass = tUp.includes('ACABADOS') ? 'group-charts group-acabados' : (tUp.includes('PROCESO') ? 'group-charts group-proceso' : 'group-charts');
                ...
                const html = `<div class="charts charts-cofaco"><div class="charts-col charts-col-left">${groupHtml('COSTURA (Equipos 1-29)', sums.costura)}</div><div class="charts-col charts-col-right">${groupHtml('ACABADOS (Equipos 30-40)', sums.acabados)}${groupHtml('Proceso (Equipos 41-69)', sums.proceso_41_69 || { cant: 0, defectos: 0, total: 0, a1: 0 })}</div></div>`;
                ...
                try {
                    ...
                    if (groups && groups.length > 2) {
                        const procesoGroup4169 = groups[2];
                        const leftCenterProceso = procesoGroup4169.querySelector('.donut-row .donut:first-child .center');
                        if (leftCenterProceso) {
                            leftCenterProceso.style.cursor = 'pointer';
                            leftCenterProceso.setAttribute('title', 'Ver detalle por equipo (Equipos 41-69)');
                            leftCenterProceso.addEventListener('click', function (ev) {
                                ev.stopPropagation();
                                if (window.latestFilteredData && window.latestFilteredData.perEquipo) {
                                    showModalDefects(window.latestFilteredData.perEquipo, 41, 69);
                                } else {
                                    alert('No hay datos detallados disponibles. Aplica el filtro primero.');
                                }
                            });
                        }
                        const rightCenterProceso = procesoGroup4169.querySelector('.donut-row .donut:nth-child(2) .center');
                        if (rightCenterProceso) {
                            rightCenterProceso.style.cursor = 'pointer';
                            rightCenterProceso.setAttribute('title', 'Ver %BAP por equipo (41-69)');
                            rightCenterProceso.addEventListener('click', function (ev) {
                                ev.stopPropagation();
                                if (window.latestFilteredData && window.latestFilteredData.perEquipo) {
                                    showBapModal(window.latestFilteredData.perEquipo, 41, 69);
                                } else {
                                    alert('No hay datos detallados disponibles. Aplica el filtro primero.');
                                }
                            });
        ...
        async function loadAndCompute(forceReload) {
            try {
                // Caché de 5 min: los cambios de filtro recalculan en memoria sin
                // volver a descargar la hoja (antes CADA cambio re-descargaba todo).
                // El botón ⟳ fuerza la recarga con forceReload=true.
                let data = null;
                const cacheValido = forceReload !== true && _dataCache.rows && (Date.now() - _dataCache.loadedAt) < DATA_TTL_MS;
                if (cacheValido) data = _dataCache.rows;
                ...
                // Guardar en caché lo recién descargado
                if (!cacheValido && data && data.length > 0) {
                ...
                // attach change handlers (bind único con dataset.bound: loadAndCompute
                // se re-ejecuta en cada cambio de filtro y antes acumulaba un juego de
                // listeners por corrida, disparando recargas múltiples en cascada)
                ...
                data.forEach(row => {
                    ...
                    const group = (equipoNum >= 1 && equipoNum <= 29) ? 'costura' : (equipoNum >= 30 && equipoNum <= 40) ? 'acabados' : (equipoNum >= 41 && equipoNum <= 69) ? 'proceso_41_69' : null;
                    if (!group) return;
                    ...
                    // --- COFACO accumulation ---
                    if (isCofaco) {
                        if (group === 'costura') costuraRows++; else if (group === 'acabados' || group === 'proceso_41_69') acabadosRows++;
                ...
                const aRowProceso = document.getElementById('proceso-41-69-row');
                const nfPct = new Intl.NumberFormat('es-ES', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
                const nfBap = new Intl.NumberFormat('es-ES', { minimumFractionDigits: 1, maximumFractionDigits: 1 });
                ...
                if (aRowProceso) {
                    aRowProceso.cells[1].textContent = nfNum.format(sums.proceso_41_69.cant);
                    aRowProceso.cells[2].textContent = nfNum.format(sums.proceso_41_69.defectos);
                    const pctProceso = (sums.proceso_41_69.cant && sums.proceso_41_69.cant > 0) ? (sums.proceso_41_69.defectos / sums.proceso_41_69.cant * 100) : null;
                    aRowProceso.cells[3].textContent = pctProceso === null ? '–' : (nfPct.format(pctProceso) + '%');
                    aRowProceso.cells[4].textContent = nfNum.format(sums.proceso_41_69.total);
                    aRowProceso.cells[5].textContent = nfNum.format(sums.proceso_41_69.a1);
                    const bapProceso = (sums.proceso_41_69.total && sums.proceso_41_69.total > 0) ? (sums.proceso_41_69.a1 / sums.proceso_41_69.total * 100) : null;
                    aRowProceso.cells[6].textContent = bapProceso === null ? '–' : (nfBap.format(bapProceso) + '%');
                }
        ...
        // Restaurar la última selección de filtros antes de la primera carga:
        // loadAndCompute ya conserva FILTER_YEAR / PERIOD_TYPE / PERIOD_VALUE
        // cuando siguen disponibles en los datos, así que basta con fijarlos.
        (function restoreSavedFilters() {
            const saved = readSavedFilters();
            if (!saved) return;
            if (saved.year && !isNaN(Number(saved.year))) FILTER_YEAR = Number(saved.year);
